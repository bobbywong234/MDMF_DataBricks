...
# MAGIC %run "/Shared/Metadata Driven Ingestion Framework/Tools/Development/utilities"
      ...
      try:
        ...
        if result_data['unioned_df'].count() != 0:
          ...
          # writing valid data
            ...
            save_to_delta_format(correctSpk,'datavalidation',storage_account_name, SinkValid, write_mode, input_parameters) # save to delta

          if len(DF_corrupted) > 0:
          # writing invalid data
            corrupt = DF_No_corrupted.fillna(np.nan)
            corruptSpk = spark.createDataFrame(corrupt)
            save_to_delta_format(corruptSpk,'datavalidation',storage_account_name, SinkInvalid, write_mode, input_parameters)
      ...
      try:
        ...
        if sink_regex_array['corrupted_indexes']:
          ...
          if len(no_corrupted) > 0:
            ...
            save_to_delta_format(no_corrupted_Spk,'datavalidation',storage_account_name, SinkValid, write_mode, input_parameters)

          # Corrupted data
          corrupted = corrupted.fillna('')
          corrupted_Spk = spark.createDataFrame(corrupted)
          save_to_delta_format(corrupted_Spk,'datavalidation',storage_account_name, SinkInvalid, write_mode, input_parameters)
...
#### SCHEMA DRIFT VALIDATION
  ...
  def orphans(self,*args,**kwargs):
    ...
    try:
      ...
      save_to_delta_format(dfValid,'datavalidation',storage_account_name, output_path+'/Validated', write_mode, input_parameters)
      self.dataframe=dfValid
      dfInValid = spark.sql(sqlQueryInvalid)
      
      save_to_delta_format(dfInValid,'datavalidation',storage_account_name, output_path+'/Invalid', write_mode, input_parameters)
